...
**iframe** 是 HTML 的一个标准元素，用于在当前页面中**嵌入另一个独立的 HTML 文档**，其内部是一个**完整的、独立的浏览器环境**，拥有自己的 `window`、`document`、历史记录、cookie 等。
...
iframe 是早期前端的技术，在现代前端开发中，**除非特殊场景，否则一律不推荐**。主要原因如下：

- **重复加载资源**：iframe 会完整加载一套独立的 HTML、CSS、JS、图片，甚至重复加载父页面已有的资源，极大浪费带宽和内存。
- **可能阻塞主页面加载**：默认情况下，父页面的 `onload` 事件需要等待所有 iframe 加载完毕后才会触发。但可通过 `loading="lazy"` 或动态创建 iframe 的方式避免阻塞。
- **移动端体验极差**：iframe 很难完美适配不同手机屏幕，容易出现双滚动条、触摸事件冲突等问题。
- **书签/前进后退失效**：iframe 内的页面跳转不会改变浏览器地址栏，用户无法收藏当前状态，也无法使用前进/后退按钮。
- **SEO 不友好（致命缺点）**：主流搜索引擎（百度、谷歌）很难抓取 iframe 内的内容，会将其视为外部页面，不计入主页面权重。
- **通信存在限制**：父页面与 iframe 之间只能通过 `postMessage` 进行异步通信，需手动验证 `event.origin` 确保安全，且调试相对不便。
- **兼容性问题**：弹窗、文件上传、路由跳转在 iframe 中经常出现异常行为。
- **安全风险（点击劫持）**：攻击者可通过透明 iframe 覆盖在按钮上诱骗用户点击，需通过 CSP 或 `X-Frame-Options` 防护。需注意：跨域 iframe 受同源策略保护，**无法读取父页面的 DOM 或 cookie**。
- **Cookie 策略复杂**：跨域 iframe 的 Cookie 携带行为因浏览器而异。Chrome 要求设置 `SameSite=None; Secure` 才能携带；Safari 默认阻止第三方 Cookie；Firefox 部分阻止；隐私模式下基本都不携带。