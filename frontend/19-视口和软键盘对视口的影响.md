...
**场景：** van-popup + fixed 弹层，弹层内的输入框，唤起软键盘时，有概率会悬浮在 popup 上，即遮挡弹层内的内容（正常现象是，软键盘唤起，输入框自动滚动到可视区域）。

**问题排查：** 跟 css 的 user-select 的属性有关，表现为：

- 默认值 auto，容易触发遮挡情况，值改为 none 后，未发现遮挡情况。
- 默认值 auto，input 的 placeholder 属性有值，容易出现遮挡情况；placeholder 不设置或设置为空，未发现遮挡情况。

**原因：**这是 iOS 上常见的 `position: fixed` 弹窗在软键盘弹出时的布局问题，可能原因：

- 选择行为与键盘响应的耦合：点击输入框会出现文本选择菜单（当 user-select: auto/text）、放大镜光标控制（移动光标位置），这些交互可能与键盘弹出逻辑存在隐含的关联。
- iOS Safari 的 "滚动优化" 机制：在检测到可选择的文本输入时，会采用不同的滚动策略（自动滚动、标准键盘处理）。

**解决**：设置 `user-select: none;`。
