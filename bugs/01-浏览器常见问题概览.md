...
Chrome 浏览器调试界面，断点不会在 debugger 语句那一行停下，原因可能有：

* 代码问题：代码逻辑跑不到 debugger 那一行。

* 编译问题：项目构建时，自动删除了 debugger 语句。
* 定位问题：项目构建未生成 source-map 文件，导致 debugger 定位不准。
* 配置问题：Chrome 开发者调试工具配置了某种忽略文件的规则（比如，鼠标在 source 面板的文件中，误点了右键菜单栏的 『Add script to ignore list』），具体规则可在调试工具 -> 设置 -> Ignore list 面板中查看。

##### debugger 定位不准

Chrome 浏览器调试界面，断点停下来的地方并不是 debugger 语句那一行。有时候差一点，有时候差很多。

**描述：** 在 vue 项目开发时，出现 debugger 定位不准。
...
**注意：** iOS需要 2 次调用方法，才能保证图片正确生成（dom-to-image，也有一样问题）。https://github.com/bubkoo/html-to-image/issues/52
