...
#### git push，报错：fatal: Could not read from remote repository.
...
* 找到  `.ssh` 文件夹（如 window：C:/Users/username/.ssh）；
* 在 `.ssh` 文件夹的 `config` 文件新增如下内容（没有 config 文件，则新建一个）：
