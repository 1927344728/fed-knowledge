...
Git 是一个开源的分布式版本控制系统，是目前世界上最先进的分布式版本控制系统（没有之一）。

**Git 是用 C 语言开发的，**是 Linus Torvalds 为了帮助管理 Linux 内核开发而开发的一个开放源码的版本控制软件。它采用了**分布式版本控制系统**，而 CVS 及 SVN 都是集中式的版本控制系统。
